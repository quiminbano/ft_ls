...
/*   Updated: 2024/05/30 15:26:25 by corellan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_ls_bonus.h"

/*static size_t	get_size_alloc_acl(t_fileinfo *info, char **acl_at)
{
	size_t	length_acl;
	size_t	size_alloc;
	char	*acl;
	...
	acl = (*acl_at);
	size_alloc = 0;
	while (length_acl < (size_t)info->acl_size)
	{
...
void	process_acl(t_fileinfo *info, int *ret_err, char **acl_at)
{
	...
	/*size_alloc = get_size_alloc_acl(info, acl_at);*/
}
